...
export function normalizeDriveUrl(url) { const src = String(url ?? '').trim(); if (!src || src === 'undefined' || src === 'null') return ''; const match = src.match(/\/file\/d\/([^/?#]+)/); if (match) return `https://drive.google.com/thumbnail?id=${match[1]}&sz=w800`; return src; }
